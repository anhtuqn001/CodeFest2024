...
class GameMap {
  ...
  run() {
    const gameLoop = () => {
      if (this.gameLock === false) {
        ...
        if (
          this.waitForNextStop &&
          Date.now() - this.haltSignalTime > TimeInOneCell
        ) {
        ...
        if (!this.roadMap.length) {
          ...
          if (this.indanger) {
            ...
            this.specialSpot &&
            this.reachableCells.has(this.specialSpot)
          ) {
  ...
  parseTicktack(id, res) {
    ...
    const currentPlayer = mapInfo.players.find((p) =>
      this.playerId.includes(p.id)
    );
    this.player = new GamePlayer(this, currentPlayer);
    const opponents = mapInfo.players.filter(
    ...
    const hasNewBomb =
      this.newOpponentBombs.filter(
        (b) => this.oldOpponentBombs.indexOf(b) === -1
      ).length > 0;
    this.oldOpponentBombs = this.newOpponentBombs;
    if (
      hasNewBomb &&
      this.roadMap.filter((c) => this.bombSpots.has(c)).length
    ) {
      this.haltSignal = true;
      this.socket.emit("drive player", { direction: "x" });
      this.haltSignalTime = Date.now();
    ...
    this.endGame =
      EnableEndGame &&
      this.flatMap.filter((c) => c === MapCell.Balk).length <= 5;
  ...
  recheckCanBomb() {
    const canBomb =
      this.mapInfo.bombs.filter((b) => this.playerId.includes(b.playerId))
        .length === 0;
    if (canBomb) {
  ...
  goBomb() {
    ...
    if (this.spot1) {
      ...
      Date.now() - this.idleStart > TimeInOneCell * 22 &&
      this.gstEgg3
    ) {
  ...
  adjustDirectionToDestroyBalk(standNode, direction) {
    ...
    return this.adjustDirection(standNode, direction);
  }
  ...
  getAvoidBomb(
    ...
    this.scanRawMap(
      ...
      (currentNode) => {
        ...
        if (
          !bombSpots.has(loc) ||
          (withTeleport && map[loc] === MapCell.TeleportGate)
        ) {
    ...
    while (queue.length) {
      ...
      for (let idx in neighbors) {
        ...
        if (
          ...
          cellValue === MapCell.God ||
          (withTeleport && cellValue === MapCell.TeleportGate)
        ) {
          if (!visited.has(neighbor)) {
            visited.add(neighbor);
            const dir = parseInt(idx, 10) + 1;
            const neighborNode = new TreeNode(
              neighbor,
              dir.toString(),
              currentNode,
  ...
  checkForSpecialSpot(map) {
    for (let c in map) {
      ...
      if (this.player?.playerInfo?.hasTransform) {
        this.specialSpot = null;
      }
      if (
        this.player?.playerInfo?.hasTransform === false &&
        cellType === MapCell.God
      ) {
        this.specialSpot = parseInt(c, 10);
      }
    }
  ...
  goExploreSpecialSpot() {
    ...
    const map = this.flatMap;
    const node = this.scanRawMap(
      new TreeNode(this.player.position),
      map,
      (currentNode) => {
        const loc = currentNode.val;
        if (this.opponentPositions.has(loc)) {
  ...
  isFullPower() {
    return (
      checkFullPower &&
      this.player.playerInfo.dragonEggSpeed >= 2 &&
